...
//		  req.app.db.models.Account ,req.user.roles.account.integral;
	  ...
	  Integral.convertCode(200 ,function(err ,result){
		  if(err){
			  console.log(err);
		  }
		  if(result){
			  ...
			  req.app.logger.log(req.app, req.user.username, req.app.reqip.getClientIp(req), 'SUCCESS', 'convertCode', '用户' + req.user.username + '使用防伪码: testing 兑换积分成功.(原消费总消费金额:' + result.consumeMoney + ',新增消费金额:' + result.addMoney + ',新总消费金额:' + req.user.roles.account.integral.consumeMoney + ';原总积分:' + result.points + ',新增积分:' + result.addPoints + ',新总积分:' + req.user.roles.account.integral.points + ';原等级:' + result.levelName + ',新增等级:' + result.addLevel + ',新等级:'+ req.user.roles.account.integral.levelName +')');
